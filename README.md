...
| `/regista <name>` | `/director` | Look up a director on TMDb and browse their full filmography (features and shorts, each sorted by a Bayesian weighted rating — favors well-established, well-rated films over trending-but-thin-data new releases; the ⭐rating(votes) is shown next to each title), with a checkbox picker (+ separate "select all" for films and for shorts) to batch-request whichever titles you want. Features go through Radarr (same as `/scarica`); shorts almost never have torrent indexer releases, so they're routed to `/cerca`'s archive.org + YouTube search instead. |
